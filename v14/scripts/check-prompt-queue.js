...
const DEPENDENCY_CHECKS = {
  'Supabase project `mpxgyobotiqcawmqlhbf` accessible': async () => {
    ...
    // Could add actual connectivity check here if needed
    return { ok: true };
  },
  ...
  'Supabase `psnm_customers` table schema stable': async () => {
    // Placeholder: would need actual DB query to verify schema
    // For now, just check if env vars exist
    const supabaseUrl = process.env.SUPABASE_URL;
  ...
  'SendGrid magic link template configured': async () => {
    // Placeholder: would need SendGrid API call to verify template exists
  ...
  'Booking system decision made': async () => {
    // This is a manual decision, can't verify programmatically
    return { ok: true, note: 'Manual verification needed' };
  },

  'n8n instance accessible': async () => {
    // Placeholder: would need n8n API check
  ...
  'Granola app active': async () => {
    // Can't verify if Granola is active programmatically
    return { ok: true, note: 'Manual verification needed' };
  },

  'YouTube channel access': async () => {
    // Would need YouTube API check
    return { ok: true, note: 'YouTube API check not implemented' };
  },

  'Social media APIs configured': async () => {
    // Placeholder for Instagram/Twitter API checks
  ...
  'Mobile testing device available': async () => {
    // Can't verify device availability
    return { ok: true, note: 'Manual verification needed' };
  },
...
function parsePromptQueue(content) {
  ...
  for (const section of sections) {
    ...
    // Only process "READY TO FIRE" section
    if (!heading.includes('READY TO FIRE')) continue;

    // Find all prompts (start with ###)
...
function parsePrompt(section) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('**Estimated time:**')) {
      ...
      // Read dependencies (next lines starting with '- ' until blank line or next section)
